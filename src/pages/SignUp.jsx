...
const SignUp = ({ embedded = false }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!nameError && !emailError && !passwordError && !confirmPasswordError) {
      try {
        // Lightweight deliverability check before signup (optional)
        try {
          const resp = await fetch(`https://api.eva.pingutil.com/email?email=${encodeURIComponent(formData.email)}`);
          const data = await resp.json();
          const isDeliverable = data?.data?.deliverable === true;
          if (!isDeliverable) {
            // Show a warning but still allow registration
            console.warn('Email deliverability check failed, but proceeding with registration');
          }
        } catch (error) {
          console.warn('Email deliverability check failed, proceeding with registration:', error);
        }
        ...
        if (result && result.success) {
          if (result.emailSent) {
            ...
            alert('⚠️ Registration successful, but email verification failed. Please contact support.');
            navigate('/login');
          }
        } else {
          console.error('Signup failed or returned unexpected result:', result);
          setErrors(prev => ({ ...prev, general: 'Registration failed. Please try again.' }));
        }
      } catch (error) {
        console.error('Registration error:', error);
        setErrors(prev => ({ ...prev, general: `Failed to create account: ${error.message}` }));
      }
    }
  };
